...
import { Send, CheckCircle2, AlertCircle, Loader2, Sparkles, Building2, MapPin, Briefcase, UserCheck, ShieldCheck } from 'lucide-react';
import { OPPORTUNITY_CATEGORIES, INQUIRER_TYPES, WORK_MODES, INDIAN_STATES, AGENCY } from '../utils/constants';
...
export default function ContactSection() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        setSuccessMessage(data.message || 'Your inquiry/application has been received successfully.');
        confetti({
          particleCount: 70,
          spread: 60,
          origin: { y: 0.6 },
        ...
        setErrorMessage(data.error || 'Unable to submit your application. Please check all fields.');
      }
    } catch (err) {
      console.error('Submission error:', err);
      setErrorMessage('Network connection error. Please ensure the backend server is running and try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <section id="contact" className="py-24 px-4 sm:px-6 lg:px-8 bg-slate-50/70 border-b border-slate-200/60 relative">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-slate-900 tracking-tight font-display">
            Get in Touch with <span className="text-gold-gradient">Vibe Media Networks</span>
          </h2>
          <p className="text-slate-600 text-sm sm:text-base mt-3 leading-relaxed">
            All candidates, content creators, and brand partners are requested to reach out exclusively through our official contact portal below.
          </p>
        </div>
        ...
        <div className="bg-white border border-slate-200/90 rounded-3xl p-6 sm:p-10 shadow-card">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {OPPORTUNITY_CATEGORIES.map((cat) => (
                <button
                  type="button"
                  key={cat}
                  onClick={() => setFormData((prev) => ({ ...prev, opportunityCategory: cat }))}
          ...
          {successMessage && (
            <div className="mb-8 p-6 rounded-2xl bg-emerald-50 border border-emerald-200 flex items-start gap-4 animate-fade-in">
              ...
              <div>
                <h4 className="text-base font-bold text-emerald-900 font-display">Submission Confirmed!</h4>
                <p className="text-xs sm:text-sm text-emerald-800 mt-1">{successMessage}</p>
                <p className="text-xs text-emerald-700 mt-2 font-semibold">Our team will review your application and respond within 24 to 48 hours.</p>
              </div>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <h3 className="text-sm font-extrabold uppercase tracking-wider text-slate-900 mb-4 pb-2 border-b border-slate-100 flex items-center gap-2">
                <span className="w-5 h-5 rounded-full bg-gold-100 text-gold-800 text-xs flex items-center justify-center font-bold">3</span>
            ...
            <div>
              <h3 className="text-sm font-extrabold uppercase tracking-wider text-slate-900 mb-4 pb-2 border-b border-slate-100 flex items-center gap-2">
                <span className="w-5 h-5 rounded-full bg-gold-100 text-gold-800 text-xs flex items-center justify-center font-bold">4</span>
            ...
            <div className="pt-4">
              ...
              <p className="text-center text-[11px] text-slate-500 mt-3 font-medium">
                🔒 All submissions are securely routed to our management & recruiting inbox at Vibe Media Networks.
              </p>
            </div>
